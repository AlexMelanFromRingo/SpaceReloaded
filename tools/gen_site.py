...
<section>
<h3>Тепло, энергоспутники, астероиды</h3>
<p>У каждого тела своя температура с суточным размахом: Луна без атмосферы качается
от жара к морозу, у Марса холодно. Экстремальная среда не бьёт по игроку лишний раз,
а нагружает <b>климат-контроль базы</b>: контроллер атмосферы на горячем или холодном
теле тратит больше энергии, чтобы держать зону, поэтому важна крепкая энергетика.</p>
<p><b>Энергоспутник</b> разворачивается на орбите как обычный спутник и питает наземные
<b>ректенны</b> при чистом небе над ними: пассивная мощность независимо от времени суток,
масштабируется с числом спутников. <b>Пояс астероидов</b> достижим с орбиты Земли: почти
невесомость, а порода при добыче даёт титан, вольфрам, метеоритное железо и лёд.</p>
</section>
