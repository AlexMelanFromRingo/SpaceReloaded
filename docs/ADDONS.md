...
| `role` | `engine`, `tank`, `command`, `seat`, `gyro`, `clamp`, `hull`, `separator` | — |
...
| `gyro_torque_nm` | компенсирующий момент; он же отрабатывает команды наклона пилота | gyro |
...
Роль `separator` — разделитель ступеней: у детали только масса, а её ярус
задаёт горизонтальную плоскость отделения. Всё на этом ярусе и ниже (включая
сам разделитель) становится нижней ступенью; ступени нумеруются снизу вверх и
зажигаются по очереди. Сборка отклонит разделитель на уровне командного модуля
или выше, пустую ступень под разделителем и деталь верхней ступени, которая
держится только на нижней. Радиальные (боковые) ускорители не поддерживаются:
ступени разделяются только горизонтальными плоскостями.
...
```jsonc
{
  ...
  // Атмосфера как физика (Полёт 2.0). Плотность у поверхности в кг/м³:
  // Земля 1.225, Марс 0.020; без поля (или 0) — вакуум, сопротивления и
  // входного нагрева нет. Действует на ракеты, ломы орудия и метеориты.
  "atmosphere_density": 0.020,

  // Высота шкалы в ИГРОВЫХ метрах: плотность ρ(h) = ρ₀·exp(−h/H) над уровнем
  // отсчёта. Вертикаль мира сжата (переход на 240–450 м), поэтому у Земли
  // H = 110, у Марса 150: атмосфера кончается до высоты перехода.
  "scale_height": 150.0,

  // Уровень отсчёта высоты (поверхность / «уровень моря» тела).
  "datum_y": 64.0,
